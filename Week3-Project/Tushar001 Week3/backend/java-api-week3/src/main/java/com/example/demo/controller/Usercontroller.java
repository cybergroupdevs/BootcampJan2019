...
public class Usercontroller {
	...
	public List<User> allProjects(){
		...
		ResultSet rs = null;
		try {
			String dbURL = "jdbc:sqlserver://CYG337;database=DB";
			String user = "sa";
			con = DriverManager.getConnection(dbURL, user, "login");
			if(con != null) {
	...
	public List<User> showOne(@PathVariable String Id){
		...
		try {
			String dbURL = "jdbc:sqlserver://CYG337;database=DB";
			String user = "sa";
			con = DriverManager.getConnection(dbURL, user, "DB");
			if(con != null) {
				Statement stmt = con.createStatement();
	...
	public void addProject(@RequestBody User user) {
		...
		try {
			String dbURL = "jdbc:sqlserver://CYG337;database=DB";
			String userdata = "sa";
			con = DriverManager.getConnection(dbURL, userdata, "DB");
			if(con != null) {
				Statement stmt = con.createStatement();
	...
	public void updateProject(@RequestBody User user, @PathVariable String id) {
		...
		try {
			...
			if(con != null) {
				...
				//update Project set name = 'project' where ID = 1;
				stmt.execute(query);
			}
		}
		catch(Exception e) {
