$(document).ready(function(){
  ...
  $.ajax({url: "http://localhost:63099/api/employees/", success: function(result){
      ...
      for(var i=0;i<result.length;i++){
        ...
        var designation = result[i].designation.name;
        ...
        for(var i=1;i<trs.length;i++){
            ...
            for(var j=2;j<3;j++){
              ...
              tlink.addEventListener('click',function(value) {
                ...
                $.ajax({url:"http://localhost:63099/api/employees/"+id,success:function(result){
                  console.log(result);
                }});
...
function del(emp_id){
    console.log(emp_id);
    var url = "http://localhost:63099/api/employees/"+emp_id;
    console.log(url);
    $.ajax({
      url:url,
      type: 'DELETE',
